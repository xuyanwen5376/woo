...
class IMManager {
  ...
  Future<void> init({
    ...
    try {
      ...
      // 会话监听
      _manager.v2TIMConversationManager.addConversationListener(
  ...
  Future<void> cleanListener() async {
    ...
    await _manager.v2TIMConversationManager
  ...
  Future<V2TimConversation> getConversation(String cid) async {
    ...
    try {
      final result = await _manager.v2TIMConversationManager.getConversation(
        conversationID: cid,
      );
      if (result.code != 0 || result.data == null) throw result.desc;
      return result.data!;
  ...
  Future<void> deleteConversation(String cid) async {
    try {
      final result = await _manager.v2TIMConversationManager.deleteConversation(
        conversationID: cid,
      );
  ...
  Future<int> getUnreadCount() async {
    ...
    try {
      final result =
          await _manager.v2TIMConversationManager.getTotalUnreadMessageCount();
      if (result.code != 0) {
        count = 0;
      } else {
        count = result.data ?? 0;
  ...
  /// 删除本地消息
  /// [msgID] 消息 msgID
  Future<int> deleteMessageFromLocalStorage({
    required String msgID,
  }) async {
    try {
      final result =
          await _manager.v2TIMMessageManager.deleteMessageFromLocalStorage(
        msgID: msgID,
      );
      if (result.code != 0) throw result.desc;
      return result.code;
  ...
  Future<int> deleteMessages({
    ...
    try {
      final result = await _manager.v2TIMMessageManager.deleteMessages(
        msgIDs: [msgID],
      );
      if (result.code != 0) throw result.desc;
      return result.code;
